...
export const Navbar = () => {
  ...
  const CartModel = ({ setIsCartOpen }) => {
    ...
    const totalCartPrice = memoizedCartItems
      .filter(item => item?.product?._id)
      .reduce((sum, item) => {
        const price = item.price || item.variantDetails?.price || item.product?.buyPrice || item.product?.variants?.[0]?.price || 0;
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                    ...
                    <h4 style={{
                      ...
                      ${(item.price || item.variantDetails?.price || item.product?.buyPrice || item.product?.variants?.[0]?.price || 0).toFixed(2)}
                      {item.quantity > 1 && (
                        <span style={{ fontSize: "0.9rem", fontWeight: "400", color: "#666" }}>
                          {" "}(Total: ${((item.price || item.variantDetails?.price || item.product?.buyPrice || item.product?.variants?.[0]?.price || 0) * item.quantity).toFixed(2)})
                        </span>
                      )}
                    </h4>
